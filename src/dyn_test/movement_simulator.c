 ...
 * @param milliseconds Number of milliseconds required to have passed to return true
...
/**
 * Reads from the dynamixel memory the speed of an endless turning wheel
 *
...
void _speed_dyn_2_speed_int(int16_t *v, uint8_t motor_id) {
    *v = dyn_mem[motor_id][DYN_REG__MOV_SPEED_L];
    *v |= ((dyn_mem[motor_id][DYN_REG__MOV_SPEED_H] & 0x03) << 8);
    if (dyn_mem[motor_id][DYN_REG__MOV_SPEED_H] & 0x04) {
        *v *= -1;
    }
}
...
void calculate_new_position() {
    ...
    if (robot_pos_str.iv_l == robot_pos_str.iv_r) {
        ...
        robot_pos_str.icc_x = robot_pos_str.x_p - robot_pos_str.r * sin(robot_pos_str.theta);
        robot_pos_str.icc_y = robot_pos_str.y_p + robot_pos_str.r * cos(robot_pos_str.theta);
...
void check_collision() {
    if (obstaculo(robot_pos_str.x, robot_pos_str.y, robot_pos_str.world)) {
        printf("***** COLLISION DETECTED AT (%u, %u) simulator step %lu\n", robot_pos_str.x, robot_pos_str.y,
               robot_pos_str.sim_step);
    }
}
